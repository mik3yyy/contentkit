...
async function runQueue(
  ...
  for (let b = 0; b < allIds.length; b += BATCH) {
    ...
    const { urls } = await res.json() as { urls: { id: string; url: string; title: string; type: string }[] }
    const pending = [...urls]
    const worker = async () => {
...
function AssetCard({
  ...
  const thumbnail = (
    ...
    >
      ...
      {/* Select checkbox (selection mode) */}
      {selecting && (
      ...
      {!selecting && (
        <>
          ...
          <button onClick={stopAndFavorite}
            className={`absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all ${favorited ? "bg-red-500 opacity-100" : "bg-black/30 backdrop-blur-sm hover:bg-black/50"}`}>
  ...
  if (selecting) {
    return (
      <div className="group cursor-pointer" onClick={() => onToggle(item.id)}>
        {thumbnail}
        {meta}
      </div>
    )
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-black text-white rounded-2xl px-6 py-4 shadow-2xl min-w-[360px] max-w-[480px]">
      ...
      <p className="text-[11px] text-gray-400">
        {finished
          ? (failed > 0 ? `${failed} file(s) could not be downloaded.` : "All files downloaded successfully!")
...
export default function LibraryClient({
  ...
  return (
    <div className="max-w-[1300px] mx-auto px-8 py-8">
      ...
      {/* Horizontal type tabs — scrollable for future expansion */}
      <div className="flex items-center gap-1 mb-6 border-b border-gray-100 pb-0 overflow-x-auto scrollbar-none">
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3">
          {items.map(item => (
            <AssetCard key={item.id} item={item} selecting={selecting} selected={selected.has(item.id)} onToggle={toggleSelect} />
          ))}
        </div>
      )}
      ...
      {/* Download overlay */}
      {dlState && (
        <DownloadProgress state={dlState} onClose={() => setDlState(null)} onCancel={cancelDownload} />
      )}
    </div>
  )
}
